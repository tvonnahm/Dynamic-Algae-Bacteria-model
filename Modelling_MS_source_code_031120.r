...
Pcref <-   0.8#0.99 (OPT with bact) 0.95# 0.99 0.9#0.9#0.9 # (TUNE) 1.5/3.5
...
Rc <- 0.01 # initial tuning 0.02 !!!###### 0.035 # (0.02 opt with bact)(TUNE) 0.025
xf <- 0.06
...
thetanmax <- 1.7 # (opt with bact/ measurd 1.7)(measured) #g chl a / g N
...
Qmax <- 0.3 #(0.29 opt with bact)(0.3 measured) 0.17/024
# Alfchl - Chl specific linitial C assimilation rate (parameter)
Alfchl <- 0.076 # initial tuning 0.1 !!! #### (0.1 opt with bact)0.1 # 0.075 (TUNE) # for Pavlova lutheri 1
...
n <- 3.45 # initial tuning 3.7 !!! ########3.7#3#3.0# (3.7 opt with bact)4 #(TUNE) 1
# kn  - half saturation constant for nitrogen uptake (parameter)
kn <- 2 #initial tuning 5 !!! #### (5 opt with bact) 2 #(TUNE) 2/3 # kn for NO3
kn2 <- 4# (14 Opt with bact) kn for NH4
nh4thres <- 8#2.6#2.6 # (8 opt with bact)
#Qsmax <-0.13
#Qsmin <-0.0037
#ns <-1
#Rs <- Rc
...
smin <- 1.82 # (1.84 opt with bact) 1.5-6 #min dis Si allowing Si assimilation

rem <- 0.8 *7 # remineralisation rate of excreted don (correction for bact C)
remd <- 0.7  *7 # remineralisation rate of background don (correction for bact C)
...
cell0 <- 7980 /1000
Si0 <- 23
PSi0 <- 7980  * 104 *10e-07 *3.9  # tuned param # QminSi * Cf0  # Si per cell under Si lim: 0.65 10e-07
don0 <- 2800/16  
bact0 <- 0#0.18/1000
...
out_axenic_old <- ode(x0, times = tspan, func = geider98old, parms = paramMod, method= "rk")
out_axenic <- ode(x0, times = tspan, func = geider98old, parms = paramMod, method= "rk")
...
out_bact_old <- ode(x0, times = tspan, func = geider98old, parms = paramMod, method= "rk")
out_bact <- ode(x0, times = tspan, func = geider98old, parms = paramMod, method= "rk")
...
#png(filename="Fig4_modpap.png", width=600, height=750)
...
plot(texp, nexpmax, type='n', 
     ylab = expression(paste("Nitrogen [",mu,"g mL"^"-1","]")), 
...
##########################################################
#png(filename="Fig5_modpap.png", width=600)
...
plot(texpout, cexpout, col="black")
points(texpout, nexpout*10, col="red")
points(texpout, chlexpout*10, col="green")
points(tdinexpout, DINexpout/10, col="blue")

observed <- as.data.frame(cbind (c(rep("C",length(texpout)),rep("Chl",length(texpout)),rep("N",length(texpout)),rep("DIN",length(tdinexpout))),
                                 as.numeric(c(texpout,texpout,texp,tdinexpout)),
                                 as.numeric(c(cexpout,chlexpout*10,nexp*10,(DINexpout)/10))),stringsAsFactors = F)
colnames(observed)<-c("name","time","val")
#observed$err<- c(cerrout, nerrout, chlerrout)     #rep(1,47)
observed$time<-as.numeric(observed$time)
observed$val<-as.numeric(observed$val)

#observed <- observed[observed$time <8.2,]

####Cost function from FME

require(FME)
CostFUN <- function (pars) {
  out <- ode(x0, times = tspan, func = geider98old,#_EPS_bact_131118, 
             parms = pars, method= "rk")
  modelled<-as.data.frame(cbind(out[,1],out[,2],out[,3]*10, out[,4]*10,out[,5]/10))
...
Pars <- pars[c(2,6,8,9)]
lowbound <- c   (0.01,       0.0075,   1,   2)
highbound <- c  (0.05,        1,       4,   10)
