...
kn2 <- 9.3#4# (14 Opt with bact) kn for NH4
nh4thres <- 2.6#8#2.6#2.6 # (8 opt with bact)
SiPS <- 0.3 #Photosynthesis reduction after Si limitation
#Qsmax <-0.13
#Qsmin <-0.0037
#ns <-1
#Rs <- Rc
...
remd <-  4.9 # remineralisation rate of background don (correction for bact C)
...
Rc <- Rc + xf #0.02#0.035 # (0.02 opt with bact)(TUNE) 0.025
xf <- 0.0#0.06
...
#png(filename="Fig4_modpap_rev.png", width=600, height=750)
...
legend("topleft", legend=c("Bacteria exp","Axenic exp", "Mod + excr (Bac)","Mod + excr (Ax)", "Mod - excr (Bac)","Mod - excr (Ax)"), 
...
legend("topright", legend=c("Bacteria exp","Axenic exp", "Mod + excr (Bac)","Mod + excr (Ax)", "Mod - excr (Bac)","Mod - excr (Ax)"), 
       col=c("red","blue","red","blue","red","blue"), 
       lty=c(0,0,1,1,3,3), pch=c(1,1,NA,NA,NA,NA),
       box.lty=0, bg="transparent")
...
##########################################################
#png(filename="Fig5_modpap_rev.png", width=600)
...
#observed$err<- c(cerr, nerr, chlerr)     #rep(1,47)
...
lowbound <- c   ( 0.1,0.1,   0.5,      0.1,  0.01)
highbound <- c  ( 10, 10,    9.3,      10,   0.5)
...
#20.0000000  9.7683810  9.2999995  0.2379652  0.1146202 
#
Fit <- modFit(f = CostFUN2, p = log(Pars), method="Nelder-Mead",
              lower=log(lowbound), upper=log(highbound))
#rem       remd        kn2   nh4thres       SiPS 
#0.1000000 10.0000000  9.3000000  0.1000000  0.2440939 


#########################################################


observed <- as.data.frame(cbind (c(rep("C",length(texp)),rep("Chl",length(texp)),rep("N",length(texp)),rep("NO3",length(tdinexp)),rep("NH4",length(tdinexp))),
                                 as.numeric(c(texp,texp,texp,tdinexp, tdinexp)),
                                 as.numeric(c(cexp,chlexp*10,nexp*10,dinexp/10, nh4exp/2)),stringsAsFactors = F))
colnames(observed)<-c("name","time","val")
#observed$err<- c(cerr, nerr, chlerr)     #rep(1,47)
observed$time<-as.numeric(observed$time)
observed$val<-as.numeric(observed$val)
...
CostFUN <- function (pars) {
  out <- ode(x0, times = tspan, func = geider98v2,#_EPS_bact_131118, 
             parms = pars, method= "rk")
  modelled<-as.data.frame(cbind(out[,1],out[,2],out[,3]*10, out[,4]*10, out[,5]/10, out[,12]/2))
  colnames(modelled)<-c("time","C","Chl","N","DIN")
  costFUN<-modCost(model = modelled, obs = observed, x="time", y = "val")
}
#x0 <- c(C0, Chl0, N0, DIN0, PSi0, Si0, bact0, 0, 0, don0, nh40) # 
