...
Rc <- 0.0958 # initial tuning 0.02 !!!###### 0.035 # (0.02 opt with bact)(TUNE) 0.025
xf <- 0
...
Alfchl <- 0.04979#0.12#0.076 # initial tuning 0.1 !!! #### (0.1 opt with bact)0.1 # 0.075 (TUNE) # for Pavlova lutheri 1
...
n <- 2.57249#3.46 # initial tuning 3.7 !!! ########3.7#3#3.0# (3.7 opt with bact)4 #(TUNE) 1
# kn  - half saturation constant for nitrogen uptake (parameter)
kn <- 1 #initial tuning 5 !!! #### (5 opt with bact) 2 #(TUNE) 2/3 # kn for NO3
...
pdf(file="FigB1_modpap_rev3.pdf")# width=600)
...
legend("topleft", legend=c("BAC+ exp","BAC- exp", "G98 BAC+","G98 BAC-"), 
       col=c("black","black","red","blue"), 
       pt.bg=c("red","blue",NA,NA),
       lty=c(0,0,1,1), pch=c(21,21,NA,NA),
...
plot(texp, cexpmax/nexpmin, type='n', 
     ...
     xlab = "time [days]",ylim=c(0,45), xlim=c(0,16),
     axes=F)
axis(2, las=1, at=c(0,5,10,15,20,25,30,35,40,45))
axis(1, las=2, at=c(0,2,4,6,8,10,12,14,16))
...
polygon(c(texp, rev(texp)), c(cexpmax/chlexpmin, rev(cexpmin/chlexpmax)),
        col=rgb(1, 0, 0, 0.2), border=NA)
lines(texp, cexp/chlexp, type="p", bg="red", pch=21)
...
polygon(c(texpout, rev(texpout)), c(cexpmaxout/chlexpminout, rev(cexpminout/chlexpmaxout)),
        col=rgb(0, 0, 1, 0.2), border=NA)
lines(texpout, cexpout/chlexpout, type="p", bg="blue", pch=21)
title(main="(e)",adj=0, cex=0.75)
points(8, cexpout[8]/chloutlier, pch=8, col="red")
...
# POC:Chl plot
plot(texp, nexpmax/nexpmin, type='n', 
     ylab = expression(paste("N : Chl ratio [",mu,"g C : ",mu,"g Chl]")), 
     xlab = "time [days]",ylim=c(0,7), xlim=c(0,16),
     axes=F)
...
polygon(c(texp, rev(texp)), c(nexpmax/chlexpmin, rev(nexpmin/chlexpmax)),
        col=rgb(1, 0, 0, 0.2), border=NA)
lines(texp, nexp/chlexp, type="p", bg="red", pch=21)
...
polygon(c(texpout, rev(texpout)), c(nexpmaxout/chlexpminout, rev(nexpminout/chlexpmaxout)),
        col=rgb(0, 0, 1, 0.2), border=NA)
lines(texpout, nexpout/chlexpout, type="p", bg="blue", pch=21)
title(main="(f)",adj=0, cex=0.75)
points(8, nexpout[8]/chloutlier, pch=8, col="red")
